...
void ksc1p1e(int **M,int m, int n,int k)
{  if(k<0) return;
...
// Branching on 1's
  M1[r[0]][c[0]]=0;
  temp=freqr[r[0]]*freqc[c[0]];
  ksc1p1e(M1,m,n,k-temp);
  M1[r[0]][c[0]]=1;
...
{ //Checking for forbidden submatrices of the type M1k and M1k transpose 
 int **Gm,*hole,temp,count,min1c,min1r,f1,minf;
...
{ 
  ...
  { for(i=1;i<hole[0]-1;i++)
    ...
    M1[min1r][min1c]=0;//Flipping the minimum weight 1-entry 
    k-=minf;
