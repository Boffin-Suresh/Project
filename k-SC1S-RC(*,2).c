...
void ksc1src(int **M,int m, int n,int k)
{  if(k<0) return;
...
//Checking for the forbidden submatrix M31
  ...
  m1=3;
  n1=4;
  M1=(int**)malloc(m*sizeof(int*));//Input Matrix
...
{ for(i2=0;i2<m;i2++)
  { if(i1==i2) continue;
    ...
    { if(i1==i3) continue;
      if(i2==i3) continue;
      ri=0;ci=0;
      for(i=0;i<n1;i++)//Checking whether all the rows of M31 are present in the sumatrix induced by columns with indices i1,i2,i3
      { for(j=0;j<n;j++)
...
//Branching on rows and columns
  for(i=0;i<n;i++)
   M1[r[0]][i]=0;
  ksc1src(M1,m,n,k-freqr[r[0]]);
  for(i=0;i<n;i++)
   M1[r[0]][i]=M[r[0]][i];

  for(i=0;i<n;i++)
   M1[r[1]][i]=0;
  ksc1src(M1,m,n,k-freqr[r[1]]);
  for(i=0;i<n;i++)
   M1[r[1]][i]=M[r[1]][i];

  for(i=0;i<n;i++)
   M1[r[2]][i]=0;
  ksc1src(M1,m,n,k-freqr[r[2]]);
  for(i=0;i<n;i++)
   M1[r[2]][i]=M[r[2]][i];

  for(i=0;i<m;i++)
   M1[i][c[0]]=0;
  ksc1src(M1,m,n,k-freqc[c[0]]);
  for(i=0;i<m;i++)
   M1[i][c[0]]=M[i][c[0]];

  for(i=0;i<m;i++)
   M1[i][c[1]]=0;
  ksc1src(M1,m,n,k-freqc[c[1]]);
  for(i=0;i<m;i++)
   M1[i][c[1]]=M[i][c[1]];

  for(i=0;i<m;i++)
   M1[i][c[2]]=0;
  ksc1src(M1,m,n,k-freqc[c[2]]);
  for(i=0;i<m;i++)
   M1[i][c[2]]=M[i][c[2]];

  for(i=0;i<m;i++)
   M1[i][c[3]]=0;
  ksc1src(M1,m,n,k-freqc[c[3]]);
...
{  //Checking for forbidden submatrices of the type M1k and M1k transpose
   int **Gm,*hole,temp,count,minf,ind;
...
{ 
  ...
  { for(i=1;i<hole[0]-1;i++)
    { if(hole[i]<m && freqr[hole[i]]!=-1 && freqr[hole[i]]<minf)
       { minf=hole[i]; ind=0;}
    
    if(hole[i]>=m && freqc[hole[i]-m]!=-1 && freqc[hole[i]-m]<minf)
       { minf=hole[i]-m; ind=1;}
    }
