...
void ksc1sr(int **M,int m, int n,int k)
{  if(k<0) return;
...
//Branching on rows
  for(i=0;i<n;i++)
   M1[r[0]][i]=0;
  ksc1sr(M1,m,n,k-freqr[r[0]]);
  for(i=0;i<n;i++)
   M1[r[0]][i]=M[r[0]][i];

  for(i=0;i<n;i++)
   M1[r[1]][i]=0;
  ksc1sr(M1,m,n,k-freqr[r[1]]);
  for(i=0;i<n;i++)
   M1[r[1]][i]=M[r[1]][i];

  for(i=0;i<n;i++)
   M1[r[2]][i]=0;
  ksc1sr(M1,m,n,k-freqr[r[2]]);

}

else

{  //Checking for submatrices of the type M1k and M1k transpose
   int **Gm,*hole,temp,count,minfr;
...
{ 
  ...
  { for(i=1;i<hole[0]-1;i++)
    { if(hole[i]<m && freqr[hole[i]]!=-1 && freqr[hole[i]]<minfr)
        minfr=hole[i];
    }
    for(i=0;i<n;i++)
    { M1[minfr][i]=0;}
